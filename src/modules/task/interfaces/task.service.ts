...
export class TaskService {
    ...
    public async insertTask(task: CreateTaskDTO): Promise<any> {
        return await this.prisma.task.create({
            data: {
                ...
                // Usamos 'connect' para asegurar la relación con el ID del usuario
                user: {
    ...
    public async updateTask(id: number, user_Id: number, taskUpdated: UpdateTaskDto): Promise<any> {
    ...
    public async deleteTask(id: number, user_Id: number): Promise<void> {
        const result = await this.prisma.task.deleteMany({
            where: {
                id: id,
                user_Id: user_Id // Solo borrará si AMBOS coinciden
            }
        });

        // Si no se borró nada (count 0), es porque no existía o no era del usuario
        if (result.count === 0) {
